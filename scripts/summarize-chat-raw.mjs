...
[업무로 인정 — 매우 중요]
다음은 모두 업무이며 제외하면 안 됩니다.
- 리서치·조사·자료 수집 (브랜드 어원·역사·시장 동향·기술·고객·경쟁사·논문·기사·레퍼런스 등)
- BI·브랜드 아이덴티티·디자인·네이밍 작업
- 데이터 분석·시각화·스프레드시트 작업
- 문서 작성·정리·번역
- 코드·기능·시스템 설계·검토
- 회의·결정·전략 논의
- 학습·교육·튜토리얼 정리 (팀원 역량 강화 차원이면)

특히 최준빈은 리서치·BI 담당이므로 어원/시장/브랜드 조사가 단일 청크여도 업무입니다. 모네 하우스(고객사) 브랜드 아이덴티티, 식품 수출 시장 조사 등이 진행 중인 task 예시입니다.

[제외 대상 — 명백한 비업무만]
- 앱 내부 설정·기능 플래그·시스템 안내문
- crisis hotline·정책 안내 같은 안전 텍스트
- 단순 인사·이모지·이모티콘만 있는 메시지
- mojibake·HTML 잔해·URL만 있는 청크 (콘텐츠 없음)
- 명확한 사적·일상 잡담 (식사 메뉴 추천, 날씨 잡담 등)

[판정 원칙]
애매하면 기록을 우선합니다. "기록할 업무성 대화 없음"은 모든 청크가 명백히 비업무일 때만 사용하세요. 한 청크라도 업무 관련이면 그것 중심으로 헤드라인을 만듭니다.
