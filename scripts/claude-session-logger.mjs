...
function llmSummary(items) {
  const list = items
    ...
    .map((t) => `- ${t.slice(0, 200)}`)
    .join('\n')
  const prompt = `토타로 팀의 Claude Code 작업 세션을 사장님께 보고하는 요약을 작성합니다.
...
[입력]
한 팀원이 Claude Code 작업 세션에서 준 지시 목록입니다(순서대로).

[출력 규칙]
1. 첫 줄: 한 문장 헤드라인 (40자 내외, 평문)
   · "무엇을 했다/구현했다/배포했다/결정했다/수정했다" 중심
   · 지시문 그대로 나열 X, 결과 중심으로 압축
   · 능동형 동사 사용
   · "검토", "방법론", "수립", "도출", "방안" 같은 추상어·컨설팅 어휘 금지
   · 마크다운·별표 사용 안 함

2. 그 다음: "· "로 시작하는 불릿 2~4개
   · 한 항목당 한 줄로 핵심 성과/결정/발견
   · 구체적 entity 보존 (파일명·함수·테이블·기능·라이브러리·도구·라우트 등)
   · 완료 / 진행 중 / 막힌 부분이 구분되면 표현
   · 의사 결정이 있으면 명시 (예: "X를 Y로 결정")
   · 다음 작업이 있으면 마지막 불릿에 "→ 다음:" 으로 표시

3. 군더더기·인사말·메타 설명·코드블록 없이 헤드라인과 불릿만 출력
