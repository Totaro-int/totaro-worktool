...
export async function AreaTaskList({
  ...
  return (
    <section className="rounded-xl bg-[#101f38] p-5 ring-1 ring-[#1c3556]">
      <div className="flex items-center gap-2">
        <h2 className="text-sm font-semibold text-[#c4d2e4]">이 영역 할 일</h2>
        <span className="rounded-full bg-indigo-50 px-2 py-0.5 text-xs font-semibold text-[#35e0ff] tabular-nums">
          {open.length}
        </span>
      </div>

      {tasks.length === 0 ? (
        <p className="mt-3 text-sm text-[#6b7c96]">
        ...
        <>
          {open.length === 0 ? (
            <p className="mt-3 text-sm text-[#6b7c96]">진행 중인 할 일이 없습니다.</p>
          ) : (
          ...
          {done.length > 0 && (
            <details className="mt-2.5 border-t border-[#12233c] pt-2.5">
              <summary className="cursor-pointer list-none text-xs font-medium text-[#6b7c96]">
                완료 {done.length}건 보기
              </summary>
...
function TaskRow({
  ...
  return (
    <li className="flex items-start gap-2.5 rounded-lg px-2 py-1.5 transition-colors hover:bg-[#14263f]">
      <form action={updateTaskStatus} className="shrink-0 pt-0.5">
        ...
        <button
          ...
          className={`flex h-[18px] w-[18px] items-center justify-center rounded-full border text-[11px] leading-none transition-colors ${
              ...
              : 'border-[#24405f] text-transparent hover:border-emerald-400 hover:text-emerald-400'
          }`}
        >
          ✓
        </button>
      ...
      <div className="min-w-0 flex-1">
        <p className={`text-sm ${done ? 'text-[#6b7c96] line-through' : 'text-[#dbe7f4]'}`}>
          {task.title}
        </p>
        {task.description && !done && (
          <p className="mt-0.5 text-xs text-[#6b7c96]">{task.description}</p>
        )}
      </div>
      {task.status === 'doing' && (
      ...
      {assignee && <span className="mt-0.5 shrink-0 text-xs text-[#6b7c96]">{assignee}</span>}
    </li>
  )
}
